...
###### StateLoop
The *StateLoop* component provides the heart of the prediction algorithm: a *stateAt* function which given a *time*, computes a prediction of the application *State*. To do so, *StateLoop* maintains a set of user *Inputs* received so far, which is used to simulate the execution of the application. *Inputs* are stored an immutable list of *Events* (triplets of *Input*, time and source), sorted by time.

Semantically, every call to the the *stateAt* function implies a recursive computation of the current state. This recursion starts from the *initialState*, and successively applies the *nextState* function with the appropriate *Events* for each time unit until it reaches the current time.

Obviously, doing the complete recursion on every frame would be too expansive. However, since this recursive process uses a pure function (returning a *State* given a time and list of *Events* happening before that time), it can easily be made efficient using memoization. Indeed, In the most common case when *stateAt* is called and no new inputs have been received since the last call, the cache will hit right away, after a single recursion step. Whenever a remote input is received and inserted into the list of inputs, the recursion will take place up to the time at which this newly received input was issued. The cache will hit at that point, from which *nextState* is successively applies until the current time is reached. This is how the *rolls back* process illustrated in #stateGraph is implemented in a time efficient manner.

Regarding memory management, timing assumptions on the network allow the use of a bounded cache. Indeed, if we define a peer to be disconnected if a message takes more than one second to transit over the network, it is sufficient to retain history of the application *State* for a period of one second. Thus, the memorization is implemented using a fixed size array, which retrains the association between a time, a list of *Events*, and a *State*. Thanks to the use of immutable lists to store the *Events*, querying the cache is done using reference equality for maximum efficiency.
